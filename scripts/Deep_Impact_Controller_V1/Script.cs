 ...
 */
namespace Deep_Impact_Controller_V1
{
    ...
    public sealed class Program : MyGridProgram
    {
         ...
         */
        #region Deep_Impact_Controller_V1
        ...
        int Stage = 1;
        string Mode = "Drill";
        const float PistonDrillSpeed = 0.01f;
        ...
        const string GrinderName = ShipName + "Grinder";
        ...
        public void Main(string args)
        {
            ...
            IMyTerminalBlock Grinder = (IMyTerminalBlock)GridTerminalSystem.GetBlockWithName(GrinderName);
            ...
            {
                Mode = States[0];
                Running = Convert.ToBoolean(States[1]);
                Stage = Int32.Parse(States[2]);
                Toggle = Convert.ToBoolean(States[3]);
            }
            ...
            if (DisplayError)
            {
                Echo("Unable To Change Mode, Drill Head Must Be In Loop Starting Position First!");
            }
            Echo("Loop: " + Toggle);

            if (Running && Mode == "Drill")
            {
                if (!CompsOn)
                {
                    CompsOn = true;
            ...
            else if (Running && Mode == "Retract")
            {
                ...
                else if (Stage == 7)
                {
                    ...
                    for (int i = 0; i < Pistons.Count; i++)
                    {
                        Pistons[i].SetValue("Velocity", PistonRetractSpeed);
                    }
            ...
            Controller.CustomData = Mode + ':' + Running.ToString() + ':' + Stage.ToString() + ':' + Toggle.ToString();
        }
        #endregion // Deep_Impact_Controller_V1
    }
}